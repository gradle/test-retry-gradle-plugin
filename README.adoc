...
Each execution of a test is discretely reported in the Gradle generarted xml and html reports. For a flaky test that fails in the first run and passes in the retry, both
are reported in the test reports.
...
Similar to the Xml and Html reports, the console log will also report each individual test execution. Keep in mind that before retrying a failed test, Gradle will execute the whole test suite
of the test task. This means the same test executed multiple times will not be grouped in the console log.
