...
=== Verifying the signature of the plugin jar

_(plugin 1.1.6+)_

The plugin jar is published to plugins.gradle.org alongside its signature.
The public key is published to http://pool.sks-keyservers.net and https://keys.openpgp.org.
You can verify the signature as follows:

[source,subs="attributes,verbatim"]
----
curl -OL https://plugins.gradle.org/m2/org/gradle/test-retry-gradle-plugin/{plugin-version}/test-retry-gradle-plugin-{plugin-version}.jar
curl -OL https://plugins.gradle.org/m2/org/gradle/test-retry-gradle-plugin/{plugin-version}/test-retry-gradle-plugin-{plugin-version}.jar.asc
gpg --keyserver keys.openpgp.org --recv-key 314FE82E5A4C5377BCA2EDEC5208812E1E4A6DB0
gpg --verify test-retry-gradle-plugin-{plugin-version}.jar.asc test-retry-gradle-plugin-{plugin-version}.jar
----
...
The warning is emitted because you haven't explicitly trusted the imported key (hence `[unknown]`).
One way of establishing trust is to verify the fingerprint over a secure channel.
Please contact technical support should you wish to do so.
